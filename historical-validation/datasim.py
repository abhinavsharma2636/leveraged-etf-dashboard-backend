...
# Date range
start = sub["entry_date"].min().normalize()
...
# Price plot with trades, EMAs and BB
axs[0].plot(price_data["Close"], label=f"{ticker} Close", color="black")
axs[0].plot(price_data["ema20"], label="EMA 20", color="blue", alpha=0.6)
axs[0].plot(price_data["ema50"], label="EMA 50", color="green", alpha=0.6)
axs[0].plot(price_data["ema200"], label="EMA 200", color="magenta", alpha=0.6)
...
axs[0].set_title(f"{ticker} Price with Trades, EMAs, and Bollinger Bands")
